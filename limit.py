...
class AccessChecker(threading.Thread):
    ...
    def run(self):
        ...
        try:
            ...
            netstate_data = subprocess.run(
                ["cut", "-d:", "-f1"], input=netstate_data, capture_output=True, text=True
            ).stdout
            netstate_data = subprocess.run(
                ["sort", "|", "uniq", "-c"], input=netstate_data, capture_output=True, text=True
            ).stdout
            netstate_data = subprocess.run(
                ["sort", "-nr", "|", "head"], input=netstate_data, capture_output=True, text=True
            ).stdout
            ...
            logging.error(f"Error during access check: {e}")
